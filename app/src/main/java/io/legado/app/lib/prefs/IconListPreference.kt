...
class IconListPreference(context: Context, attrs: AttributeSet) : ListPreference(context, attrs) {
    ...
    private val typedArray: TypedArray = context.resources.obtainTypedArray(R.array.icon_id)
    ...
    init {
        ...
        for (i in 0 until typedArray.length()){
            val rid = typedArray.getResourceId(i, R.drawable.ic_launcher)
            var d: Drawable? = null
            kotlin.runCatching {
                d = context.getCompatDrawable(rid)
            }
        ...
        inner class Adapter(context: Context) :
            ...
            override fun convert(
                ...
                binding.run {
                    ...
                    val rid = typedArray.getResourceId(index, R.drawable.ic_launcher)
                    val d = try {
                        context.getCompatDrawable(rid)
                    } catch (e: Exception) {
                        null
                    }
                    d?.let {
